...
- `components/platform-picker.tsx` ('use client'):
  ...
  - Dropdown panel: search input (autofocus); list grouped by `CATEGORY_ORDER` filtered by `query` (match name/domain); each row = PlatformIcon + name, onClick -> `onChange(p.domains[0])` + close.
